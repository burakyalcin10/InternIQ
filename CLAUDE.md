...
- **Subprocess isolation for heavy LLM work**: when the MCP server runs CrewAI or the LangGraph application workflow, the openai SDK's first sync httpx request hangs in the FastMCP stdio child on Windows. Both paths therefore spawn a clean Python subprocess (`backend/crew/_run_crew_subprocess.py`, `backend/langgraph_workflow/_run_workflow_subprocess.py`) with `stdin=DEVNULL`, `stderr=DEVNULL`, UTF-8-forced stdout, and a JSON-only contract. Don't add new heavy in-process LLM tools to the MCP server without using the same pattern.
- Terminal client `prepare <listing_id>` is a smart router: if logged in with a saved CV → calls `run_profile_application_workflow`; otherwise falls back to `run_application_workflow` with passed cv_text (or empty)
- Action plan prompt (`langgraph_workflow/nodes.py` → `create_action_plan`) enforces 5 fixed step categories (Technical Gap / Portfolio / Company / Interview / Application Strategy) and forbids duplicate themes; `ChatOpenAI` uses `max_tokens=1200` to avoid mid-thought truncation in CV suggestions and interview question generation
- CrewAI agents are silent by default (`verbose=False`) so their rich/emoji output cannot leak into the MCP JSON-RPC stream — set `CREWAI_VERBOSE=1` in the env to re-enable for local debugging
